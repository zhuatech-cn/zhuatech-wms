...
新增库存锁定、拣选、装箱复核、承运计划、危险品文件、冷链准备、异常和截单时间联合门禁，详见 [出库放行治理](docs/ENTERPRISE_SHIPMENT_RELEASE.md)。

## 企业级增强：库存差异调整过账

新增盘点关闭、独立复盘、批次序列追溯、质量状态、库存权属、负库存、价值权限、成本影响、财务/业务批准、职责分离和审计证据门禁，详见[库存差异调整治理](docs/ENTERPRISE_INVENTORY_ADJUSTMENT.md)。
